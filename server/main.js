...
app.use(async (req, res, next) => {
    ...
    try {
      ...
      if (address.toLowerCase() !== signature[0].toLowerCase()) {
        writeError(
          res,
          403,
          `signer address does not equal expected, got ${address}, expected ${signature[0]}`,
        );
        return;
      }
  ...
  if (auth) {
    writeError(
      res,
      403,
      'invalid authorization method, API keys have been deprecated, use X-Flashbots-Signature',
    );
    return;
  }
